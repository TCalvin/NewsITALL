...
import {removeDislike} from '@/Utilities/articles.js'

export default {
  name: 'DislikedArticle',
  props: ['post', 'email'], //The post to be displayed
  data(){
    return{
    }
  },
  methods: {
    ...
    /**
     * Called when the modal button is pressed
     */
    removePost(title){

      removeDislike(this.email, this.post)
      .then(response => {
