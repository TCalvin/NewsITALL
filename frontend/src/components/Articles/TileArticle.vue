  ...
  <div width=296px>
    <!--Stats cards-->
        <stats-card v-bind:view="view">
          <div v-if="article.video == ''"  align="left" class="image-container" slot="header">
          ...
          <div align="left" slot="content">
            <a v-bind:href="article.URL" target="_blank"><h4 v-bind:class="sentimentColor">{{article.title}}</h4></a>

            <p class="black" v-if="article.summary.length > 200" v-html="article.summary.substring(0, 200).concat('...')"/>
            <p class="black" v-if="article.summary.length <= 200" v-html="article.summary.substring(0, 200)"/>
          </div>
